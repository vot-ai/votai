# Generated by Django 3.0.5 on 2020-05-02 00:24

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('items', '0001_initial'),
        ('surveys', '0001_initial'),
    ]

    operations = [
        migrations.CreateModel(
            name='Annotator',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('name', models.CharField(max_length=30)),
                ('active', models.BooleanField(default=True)),
                ('alpha', models.FloatField(default=10)),
                ('beta', models.FloatField(default=1)),
                ('current', models.OneToOneField(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='current_annotator', to='items.Item')),
                ('ignored', models.ManyToManyField(related_name='ignored_by', to='items.Item')),
                ('previous', models.OneToOneField(null=True, on_delete=django.db.models.deletion.SET_NULL, related_name='previous_annotator', to='items.Item')),
                ('survey', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='annotators', to='surveys.Survey')),
                ('viewed', models.ManyToManyField(related_name='viewed_by', to='items.Item')),
            ],
        ),
    ]
