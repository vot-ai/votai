...
class QueryFieldsMixin(object):
    ...
    def get_include_fields_dict(self, query_params):
        ...
        if self.is_child:
            parent_include_fields = self.get_parent().get_include_fields_dict(
                query_params
            )
        ...
        return {
            ...
            for name, val in self.process_query_fields(names).items()
            if name
        }
    ...
    def get_exclude_fields_dict(self, query_params):
        ...
        if self.is_child:
            parent_exclude_fields = self.get_parent().get_exclude_fields_dict(
                query_params
            )
            child_field_name = self.get_field_name_as_child()
